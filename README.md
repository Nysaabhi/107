        ...
        <div class="list-item" onclick="toggleDetailedInfo(this)">
            ...
            <div class="item-actions" onclick="event.stopPropagation()">
                ${item.actionButtons.map(btn => `
                    <button class="action-button" data-url="${btn.url}">
...
class ChatSystem {
  ...
  return `
    <div class="prompt-response" onclick="toggleDetailedInfo(this)">
      <div class="answer">${promptData.answer}</div>
      <div class="action-buttons">
        ${promptData.actionButtons.map(btn => `
          <button class="action-button" data-url="${btn.url}" onclick="event.stopPropagation()">
            <i class="fas ${btn.icon}"></i> ${btn.label}
          </button>
        `).join('')}
      </div>
      <div class="detailed-info hidden">
        ${Object.entries(promptData.detailedInfo).map(([section, items]) => `
          <div class="info-section">
            <h3>${section}</h3>
...
function toggleDetailedInfo(promptResponse) {
    const infoPanel = promptResponse.querySelector('.detailed-info');
    infoPanel.classList.toggle('hidden');
}
